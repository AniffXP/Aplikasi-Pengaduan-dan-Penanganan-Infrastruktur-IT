...
if (!$user) { echo "<script>alert('Data tidak ditemukan.'); window.location='../login/login.php';</script>"; exit; }
if (!isset($_SESSION['id_user'])) { $_SESSION['id_user'] = $user['id']; }
$filter_condition = "";
if ($user['level'] === 'user' || $user['level'] === 'User') {
    $id_user = intval($_SESSION['id_user']);
    $filter_condition = "WHERE laporan.id_user = $id_user";
}
$query = "SELECT laporan.*, login.nama AS user FROM laporan LEFT JOIN login ON laporan.id_user = login.id $filter_condition ORDER BY laporan.id ASC";
$result = mysqli_query($koneksi, $query);
$data = [];
if ($result && mysqli_num_rows($result) > 0) { while ($row = mysqli_fetch_assoc($result)) { $data[] = $row; } }
?>
...
<a class="btn btn-success btn-sm" href="tambah.php"><i class="bi bi-plus-circle"></i> Buat Pengaduan</a>
<div class="d-flex gap-2">
<input type="text" id="searchInput" class="form-control form-control-sm" placeholder="Cari berdasarkan kategori">
<select id="orderSelect" class="form-select form-select-sm"><option value="ASC">Urutkan A-Z</option><option value="DESC">Urutkan Z-A</option></select>
</div></div>
<div class="table-responsive text-center"><table class="table table-striped table-hover table-bordered"><thead class="table-primary text-center">
<tr><th>No</th><th>User</th><th>Kategori</th><th>Status</th><th>Prioritas</th><th>Waktu Pengerjaan</th><th>Tanggal Pengaduan</th></tr></thead>
...
tableBody.innerHTML += `<tr onclick="window.location='detail.php?id=${item.id}'" style="cursor:pointer"><td>${i+1}</td><td>${item.user||'-'}</td><td>${item.kategori}</td><td><span class="badge bg-${badge}">${item.status}</span></td><td>${item.prioritas}</td><td>${item.waktu_pengerjaan||'-'}</td><td>${item.tgl_pengaduan||'-'}</td></tr>`;});}
function filterAndSort() {
const s=searchInput.value.toLowerCase(); const o=orderSelect.value;
let fd=data.filter(i=>i.kategori.toLowerCase().includes(s));
fd.sort((a,b)=>o==='ASC'?a.kategori.localeCompare(b.kategori):b.kategori.localeCompare(a.kategori));
renderTable(fd);}
