...
if (isset($_POST['submit'])) {
    ...
    $q = "UPDATE login SET nama='$nama', user_login='$user_login', level='$level'";
    if (!empty($_POST['pass_login'])) { $q .= ", pass_login='" . mysqli_real_escape_string($koneksi, $_POST['pass_login']) . "'"; }
    $q .= " WHERE id=$id";
    if (mysqli_query($koneksi, $q)) { header("location:deprt.php"); exit; }
}
?>
<!DOCTYPE html><html lang="id"><head><title>Edit Departemen</title>
...
<h5 class="text-center text-primary mb-3" style="font-weight: 600;">Edit Departemen</h5>
<form method="POST">
<div class="mb-3"><label class="form-label">Nama Departemen</label><input type="text" name="nama" class="form-control" value="<?= htmlspecialchars($data['nama']) ?>" required></div>
<div class="mb-3"><label class="form-label">Username</label><input type="text" name="user_login" class="form-control" value="<?= htmlspecialchars($data['user_login']) ?>" required></div>
<div class="mb-3"><label class="form-label">Password (kosongkan jika tidak diubah)</label><input type="password" name="pass_login" class="form-control"></div>
<div class="mb-3"><label class="form-label">Level</label>
<select name="level" class="form-select"><option value="Admin" <?= $data['level']=='Admin'?'selected':'' ?>>Admin</option><option value="User" <?= $data['level']=='User'?'selected':'' ?>>User</option></select></div>
