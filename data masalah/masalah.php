...
?>
<!DOCTYPE html><html lang="id"><head><title>Data Masalah</title>
...
<a class="btn btn-success ms-2" href="tambah.php"><i class="bi bi-plus-circle"></i> Tambah Tracker</a></div>
<div class="table-responsive"><table class="table table-striped table-hover mx-auto text-center"><thead class="table-primary">
<tr><th>No</th><th>Tracker</th><th>Aksi</th></tr></thead>
...
tableBody.insertAdjacentHTML('beforeend', `<tr><td>${i+1}</td><td>${item.tracker}</td><td><a class="btn btn-warning btn-sm" href="haledit.php?id=${item.id}"><i class="bi bi-pencil-square"></i></a> <a class="btn btn-danger btn-sm" href="hapus.php?id=${item.id}" onclick="return confirm('Yakin?')"><i class="bi bi-trash"></i></a></td></tr>`);});}
function filterAndSort() { const s=searchInput.value.toLowerCase(); const o=orderSelect.value;
