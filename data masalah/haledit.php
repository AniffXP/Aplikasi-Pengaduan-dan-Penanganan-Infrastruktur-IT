...
$id = intval($_GET['id']);
$data = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT * FROM masalah WHERE id=$id"));
if (isset($_POST['submit'])) {
    $tracker = mysqli_real_escape_string($koneksi, $_POST['tracker']);
    mysqli_query($koneksi, "UPDATE masalah SET tracker='$tracker' WHERE id=$id");
    header("location:masalah.php"); exit;
}
?>
<!DOCTYPE html><html lang="id"><head><title>Edit Masalah</title>
...
<h5 class="text-center text-primary mb-3" style="font-weight: 600;">Edit Tracker</h5>
<form method="POST">
<div class="mb-3"><label class="form-label">Nama Tracker</label><input type="text" name="tracker" class="form-control" value="<?= htmlspecialchars($data['tracker']) ?>" required></div>
