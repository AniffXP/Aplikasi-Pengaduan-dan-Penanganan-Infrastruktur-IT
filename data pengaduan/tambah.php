...
$users = mysqli_query($koneksi, "SELECT * FROM login WHERE level='User' ORDER BY nama ASC");
...
<div class="mb-3"><label class="form-label">Nama User</label><select name="id_user" class="form-select" required>
<option value="">Pilih User</option><?php while($u=mysqli_fetch_assoc($users)){ echo "<option value='{$u['id']}'>{$u['nama']}</option>"; } ?></select></div>
